...
* @tree: pointer of pointer to the root node
...
if (!tree)
return (0);
lheight = tree->left ? -1 + (int) binary_tree_height(tree->left) : 0;
rheight = tree->right ? -1 + (int) binary_tree_height(tree->right) : 0;
